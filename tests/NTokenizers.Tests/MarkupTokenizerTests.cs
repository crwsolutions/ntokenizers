...
{
    private static (List<MarkupToken> tokens, string text) Tokenize(string markup)
    {
        var tokens = new List<MarkupToken>();
        using var stream = new MemoryStream(Encoding.UTF8.GetBytes(markup));
        var result = MarkupTokenizer.Create().ParseAsync(stream, token =>
        {
            tokens.Add(token);
    ...
    {
        var markup = "[link text](http://example.com)";
        var (tokens, text) = Tokenize(markup);
        Assert.Single(tokens);
    ...
    {
        var markup = "[link text](http://example.com \"title\")";
        var (tokens, text) = Tokenize(markup);
        Assert.Single(tokens);
        Assert.Equal(MarkupTokenType.Link, tokens[0].TokenType);
        Assert.Equal("link text", tokens[0].Value);
    ...
    {
        var markup = "![alt text](http://example.com/image.png)";
        var (tokens, text) = Tokenize(markup);
        Assert.Single(tokens);
    ...
    {
        var markup = "Text with **bold** and *italic* and `code`";
        var (tokens, text) = Tokenize(markup);
        Assert.Equal(7, tokens.Count);
        Assert.Equal(MarkupTokenType.Text, tokens[0].TokenType);
